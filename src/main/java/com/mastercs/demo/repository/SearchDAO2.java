...
public interface SearchDAO2 extends JpaRepository<Search,Integer>{

    // 添加功能，按照关键字查询title ，这里一定要把 * 用上不然出问题，猜想可能是和主键设置有关？
    //新建一个查询，名字叫findByTitle，查询语句是后面那句
    @Query(name="findByTitle", nativeQuery = true, value="select * from knowledge where id = :title")  //items是数据库baidu_search下的表items，存网页搜索结果
