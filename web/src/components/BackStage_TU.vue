      ...
      <el-main>
        <div>
          ...
          <CustomPages @size-change="handleSizeChange" @current-change="handleCurrentChange"
                       :currentPage="pageInfo.pageNum" :total="pageInfo.total"
                       :pageSize="pageInfo.pageSize"></CustomPages>
          <CustomDrawer :modal="false" @close-drawer="closeDrawer" :isDrawerDialog="isDrawerDialog"
    ...
    this.$nextTick(() => {
      ...
      editor.config.customUploadImg = function (files, insert) {
        for (var i = 0; i < files.length; i++) {
          ...
          request.post(_this.uploadPath, data)
              .then(res => {
                console.log(res)
